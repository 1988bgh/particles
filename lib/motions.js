const motions = {
  ...
  calcValue: (_el, _options, _scroll) => {
    let partScroll = Math.max(
      0,
      _scroll - _el.info.offsetHeight * _options.startPoint
    );
    let partStart = _el.info.offsetHeight * _options.startPoint;
    let partEnd = _el.info.offsetHeight * _options.endPoint;
  ...
  ratio: (_el, _scroll) => {
    ...
    offsetTop = _el.info.offsetTop;
    currentScroll = Math.max(0, _scroll - offsetTop);

    return (currentScroll / _el.info.offsetHeight).toFixed(4) * 1;
  },
};
